...
app.get('/healthz', (req, res) => {
  ...
  res.json({
    ...
    apify: apifyPool.status(),  // v3.26.0 — per-account pool balances (cached, refresh in background)
  });
});

// v3.26.0 — auth disabled (internal-only URL); /api/auth/login endpoint removed
...
// v3.26.0 — Admin endpoints removed (Admin tab gone; 0 persistent exclusions
...
/* ---------- Client lifecycle status (v3.26.0) ----------
...
(async () => {
  ...
  app.listen(PORT, () => {
    console.log(`RJP Sourcing Portal v3.26.0 listening on :${PORT}`);
